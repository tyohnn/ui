...
export const wireNextApp = (ctx, { appPath, system, icons, fonts, mode, port, example }) =>
{
    ...
    const systemBlock = [
        ...
        "",
        "/* Layer-1 font stacks on the next/font variables from layout.tsx: next/font hashes family names, so the",
        `   system's "${source.font(fonts.sans).family}" stack would never match. Same order and fallbacks as the system. */`,
        ":root {",
